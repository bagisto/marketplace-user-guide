...
::: info
Account-health thresholds and the flag reasons available to customers are configured under **Configure → Marketplace → Moderation**. See [Seller & Product Flagging](/moderation/seller-product-flagging) for the flagging workflow.
:::
...
A complete profile builds customer trust and controls how the storefront presents the store. For the custom profile fields the admin can require, see [Seller Attributes](/sellers/seller-attributes).
