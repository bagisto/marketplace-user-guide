...
The commission and seller-earning columns make it clear, per order, how much the marketplace keeps and how much the seller is owed. See [Commission Management](/commission-management) for how commission is calculated.
...
- **Create Shipment** — ship the items through one of the configured [Fulfilment Carriers](/fulfilment-carriers), including buying a label or scheduling a pickup where the carrier supports it.
...
Once a seller has fulfilled an order, the **seller payment status** tracks whether they've been paid for it. Sellers request their earnings from the admin — see [Payment Management](/payment-management) — and the admin can also settle balances in bulk through [Mass Payout Management](/mass-payout-management).

::: tip
Invoice and ship promptly — a healthy fulfilment record feeds into [Seller Account Health](/seller-account-health), which the admin uses to keep the marketplace reliable.
:::
